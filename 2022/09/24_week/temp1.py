...
def possilbeReservation(amount, check_in_date, check_out_date):
    global used_room
    for h in range(1, 4):
        cnt = 0
        flag = False
        for w in range(1, 21-amount+1):
...
if __name__ == "__main__":
    ...
    while D <= 10:
        ...
        if len(reservation_info) > 2:
            print(reservation_info)
        replies = []
        for i, reserv in enumerate(reservation_info):
            # reserv - "id": 453217, "amount": 10, "check_in_date": 104, "check_out_date": 110}
            room_number = possilbeReservation(reserv['amount'], reserv['check_in_date'], reserv['check_out_date'])
            if room_number == 0:
        ...
        if D > 200:
            break
        request_count += len(reservation_check[D])
