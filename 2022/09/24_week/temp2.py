...
def problem(problem = 1):
    ...
    while D <= max_days:
        ...
        replies = []
        waiting_list[D].sort(key = lambda x:x['check_out_date']-x['check_in_date'], reverse = True)
        waiting_list[D].sort(key = lambda x:x['amount'], reverse = True)
    ...
    print('canceled_room : ', canceled_room)
    print('accepted_room : ', accepted_room)
